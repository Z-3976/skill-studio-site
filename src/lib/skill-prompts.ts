...
const VISUAL_RULES = `
你是门店商业视觉设计师，负责把门店团购、健身卡、套餐图、产品头图和 KT 板整理成可直接用于生图的最终中文提示词。

必须遵守：
1. 风格优先参考用户提供的样板图，只借鉴风格、版式、质感，不篡改当前套餐内容。
2. 默认产品头图按 4:3 横版构图，中间核心内容控制在 1:1 安全区，避免裁切。
3. A4/A3 KT 板按竖版线下宣传物料逻辑排版。
4. 用户给的主题色是强约束，只允许做同色系的明暗、渐变和材质延展，不要明显跑色。
5. 如果上传了 Logo，要把 Logo 放在上方或视觉核心区，尽量保留原始图形特征。
6. 主标题大字突出，重点突出套餐名、价格、权益，少小字。
7. 整体要高级、清晰、有商业感，适合抖音团购、本地生活、健身房宣传。
8. 只输出最终可直接用于生图的中文提示词，不要写分析过程。
`;
...
export const buildSkillInstructions = (payload: StudioPayload) => {
  if (payload.skill === "chanping-toutu") {
    ...
    return {
      ...
      userPrompt: `
请基于以下信息，为我整理一版最终生图提示词：

${summary}

输出要求：
1. 如果用户上传了 Logo，请在提示词中明确要求保留 Logo 的结构特征，并放在上方或视觉核心区。
...
export const buildLocalFallback = (payload: StudioPayload): LocalFallbackResult => {
  if (payload.skill === "chanping-toutu") {
    ...
    return {
      type: "prompt",
      note: "已返回最终生图提示词。",
      text: `最终生图提示词：为${storeName ? `${storeName}的` : ""}${designType}制作一张高级商业宣传视觉，主内容为“${productName}”，价格重点突出“${price}”，核心权益包含“${benefitText}”。如果是头图，按 4:3 横版构图，中间核心信息控制在 1:1 安全区；如果是 KT 板，按竖版线下物料逻辑排版。Logo 放上方或视觉核心区，主标题大字突出，少小字，适合抖音团购、本地生活和健身房宣传。主色严格围绕“${themeColor}”展开，只做同色系的明暗、渐变和材质延展，不要明显跑色。整体质感高级、清晰、利落、有商业感。${extraNotes ? `补充要求：${extraNotes}。` : ""}`,
    };
  }
...
完整口播：
${hook}
我们想传递的不只是一个产品价格，而是一整套更容易坚持下来的训练体验。像${storeAdvantages}，这些不是一句“环境不错”就能带过去的，而是用户一到店就能感受到的差别。${isLightIp ? "这条内容不聊价格，也不催下单，只把门店的真实氛围和服务细节讲明白。" : ""}${isMarketing && price ? `如果你做的是营销视频，就把“${price}”拆清楚，把价值感和行动引导一起收住。` : ""}
